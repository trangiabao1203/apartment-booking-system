...
export class AssetController extends BaseController<Asset, string>(props) {
  ...
  async create(@UploadedFile() file: MulterFile, @Req() res: Request): Promise<Asset> {
    return this.assetService.singleUpload(file, res['payload'] as JwtPayload);
  }
  ...
  async uploadMultiple(@UploadedFiles() files: MulterFile[], @Req() res: Request): Promise<AssetResponseDto> {
    const { success, failed } = await this.assetService.bulkUpload(files, res['payload'] as JwtPayload);
